...
import { db } from "~/db";
import { createUserSession, getUser, getUserByUsername, login, register } from "~/services/session";
...
export default function Login() {
	...
	const [loggingIn, { Form }] = createServerAction$(async (form: FormData) => {
		...
		switch (loginType) {
			...
			case "register": {
				...
				const user = (await register({ username, password })).get();
				if (!user) {
	...
	createEffect(() => {
		if (loggingIn.result && loggingIn.result?.ok) {
			setTimeout(() => {
